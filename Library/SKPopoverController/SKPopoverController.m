...
#define SK_BASE_SDK_7_ENABLED
#endif

#ifdef DEBUG
#define SK_LOG(fmt, ...)		NSLog((@"%s (%d) : " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__)
#else
#define SK_LOG(...)
#endif

#define SK_IS_IOS_EQUAL_TO(v)                  ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] == NSOrderedSame)

#define SK_IS_IOS_GREATER_THAN(v)              ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] == NSOrderedDescending)

#define SK_IS_IOS_GREATER_THAN_OR_EQUAL_TO(v)  ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] != NSOrderedAscending)

#define SK_IS_IOS_LESS_THAN(v)                 ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] == NSOrderedAscending)


////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface UIColor (SKPopover)
...
@implementation UIColor (SKPopover)
...
@interface UINavigationController (SKPopover)
...
@implementation UINavigationController (SKPopover)
...
@interface UIViewController (SKPopover)
@end

////////////////////////////////////////////////////////////////////////////////////////////////////////

@implementation UIViewController (SKPopover)
...
@interface SKPopoverArea : NSObject
{
}

@property (nonatomic, assign) SKPopoverArrowDirection arrowDirection;
...
#pragma mark
#pragma mark - SKPopoverArea

@implementation SKPopoverArea
...
- (NSString*)description
{
    ...
    if (arrowDirection == SKPopoverArrowDirectionUp)
    {
        direction = @"UP";
    }
    else if (arrowDirection == SKPopoverArrowDirectionDown)
    {
        direction = @"DOWN";
    }
    else if (arrowDirection == SKPopoverArrowDirectionLeft)
    {
        direction = @"LEFT";
    }
    else if (arrowDirection == SKPopoverArrowDirectionRight)
    {
        direction = @"RIGHT";
    }
    else if (arrowDirection == SKPopoverArrowDirectionNone)
    {
        direction = @"NONE";
    }
...
+ (id)theme {
    ...
    SKPopoverTheme *result = nil;
    
    if (SK_IS_IOS_LESS_THAN(@"7.0")) {
        result = [SKPopoverTheme themeForIOS6];
    } else {
        result = [SKPopoverTheme themeForIOS7];
    }
...
+ (id)themeForIOS6 {
    ...
    SKPopoverTheme *result = [[SKPopoverTheme alloc] init];
...
#pragma mark
#pragma mark - UIImage (SKPopover)

@implementation UIImage (SKPopover)
...
@interface SKPopoverBackgroundInnerView : UIView
...
#pragma mark
#pragma mark - SKPopoverInnerView

@implementation SKPopoverBackgroundInnerView
...
@protocol SKPopoverOverlayViewDelegate;

@interface SKPopoverOverlayView : UIView
{
    BOOL testHits;
}

@property(nonatomic, assign) id <SKPopoverOverlayViewDelegate> delegate;
...
#pragma mark
#pragma mark - SKPopoverOverlayViewDelegate

@protocol SKPopoverOverlayViewDelegate <NSObject>

@optional
- (void)popoverOverlayViewDidTouch:(SKPopoverOverlayView *)overlayView;
...
#pragma mark
#pragma mark - SKPopoverOverlayView

@implementation SKPopoverOverlayView
...
#pragma mark
#pragma mark - SKPopoverBackgroundViewDelegate

@protocol SKPopoverBackgroundViewDelegate <NSObject>

@optional
- (void)popoverBackgroundViewDidTouchOutside:(SKPopoverBackgroundView *)backgroundView;

@end

////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface SKPopoverBackgroundView ()
{
    SKPopoverBackgroundInnerView *innerView;
    CGSize contentSize;
}

@property(nonatomic, assign) id <SKPopoverBackgroundViewDelegate> delegate;
...
- (CGRect)outerRect:(CGRect)rect arrowDirection:(SKPopoverArrowDirection)aArrowDirection;
- (CGRect)innerRect:(CGRect)rect arrowDirection:(SKPopoverArrowDirection)aArrowDirection;
- (CGRect)arrowRect:(CGRect)rect arrowDirection:(SKPopoverArrowDirection)aArrowDirection;
...
#pragma mark
#pragma mark - SKPopoverBackgroundView

@implementation SKPopoverBackgroundView
...
- (id)initWithContentSize:(CGSize)aContentSize
{
    ...
    if (self != nil)
    {
        ...
        self.arrowDirection = SKPopoverArrowDirectionDown;
        self.arrowOffset = 0;
        
        self.layer.name = @"parent";
        
        if (SK_IS_IOS_GREATER_THAN_OR_EQUAL_TO(@"6.0"))
        {
...
- (void)setArrowOffset:(float)value
{
    ...
    if (value != 0)
    {
        ...
        if (arrowDirection == SKPopoverArrowDirectionUp || arrowDirection == SKPopoverArrowDirectionDown)
        {
            value += coef * self.outerShadowOffset.width;
            value = MIN(value, CGRectGetWidth(outerRect) / 2);
        }
        
        if (arrowDirection == SKPopoverArrowDirectionLeft || arrowDirection == SKPopoverArrowDirectionRight)
        {
...
- (void)setViewController:(UIViewController *)viewController
{
    ...
    if (innerView == nil)
    {
        innerView = [[SKPopoverBackgroundInnerView alloc] initWithFrame:contentView.frame];
...
- (void)drawLayer:(CALayer *)layer inContext:(CGContextRef)context
{
    if ([layer.name isEqualToString:@"parent"])
    {
        ...
        else if (arrowDirection == SKPopoverArrowDirectionLeft || arrowDirection == SKPopoverArrowDirectionRight)
        {
            if (arrowOffset >= 0)
            {
        ...
        if (arrowDirection == SKPopoverArrowDirectionDown)
        {
        ...
        if (arrowDirection == SKPopoverArrowDirectionLeft)
        {
        ...
        if (arrowDirection == SKPopoverArrowDirectionRight)
        {
        ...
        if (arrowDirection == SKPopoverArrowDirectionNone)
        {
...
- (CGRect)outerRect:(CGRect)rect arrowDirection:(SKPopoverArrowDirection)aArrowDirection
{
    ...
    if (aArrowDirection == SKPopoverArrowDirectionUp || arrowDirection == SKPopoverArrowDirectionDown)
    {
        result.size.height -= arrowHeight;
        
        if (aArrowDirection == SKPopoverArrowDirectionUp)
        {
            result = CGRectOffset(result, 0, arrowHeight);
        }
    }
    
    if (aArrowDirection == SKPopoverArrowDirectionLeft || arrowDirection == SKPopoverArrowDirectionRight)
    {
        result.size.width -= arrowHeight;
        
        if (aArrowDirection == SKPopoverArrowDirectionLeft)
        {
...
- (CGRect)arrowRect:(CGRect)rect arrowDirection:(SKPopoverArrowDirection)aArrowDirection
{
    ...
    if (arrowHeight > 0)
    {
        ...
        if (aArrowDirection == SKPopoverArrowDirectionDown)
        {
        ...
        if (aArrowDirection == SKPopoverArrowDirectionRight)
        {
...
@interface SKPopoverController () <SKPopoverOverlayViewDelegate, SKPopoverBackgroundViewDelegate>
{
    UIViewController        *viewController;
    CGRect                   rect;
    UIView                  *inView;
    SKPopoverOverlayView    *overlayView;
    SKPopoverBackgroundView *backgroundView;
    SKPopoverArrowDirection  permittedArrowDirections;
    BOOL                     animated;
    ...
    SKPopoverAnimationOptions options;
...
- (void)dismissPopoverAnimated:(BOOL)aAnimated
                       options:(SKPopoverAnimationOptions)aAptions
                    completion:(void (^)(void))aCompletion
                  callDelegate:(BOOL)aCallDelegate;

- (SKPopoverArrowDirection)arrowDirectionForRect:(CGRect)aRect
                                          inView:(UIView*)aView
                                     contentSize:(CGSize)aContentSize
                                     arrowHeight:(float)aArrowHeight
                        permittedArrowDirections:(SKPopoverArrowDirection)aArrowDirections;

- (CGSize)sizeForRect:(CGRect)aRect
               inView:(UIView *)aView
          arrowHeight:(float)aArrowHeight
       arrowDirection:(SKPopoverArrowDirection)aArrowDirection;
...
#pragma mark
#pragma mark - SKPopoverController

@implementation SKPopoverController
...
static SKPopoverTheme *defaultTheme_ = nil;

+ (void)setDefaultTheme:(SKPopoverTheme *)aTheme
{
    defaultTheme_ = aTheme;
    
    @autoreleasepool {
        SKPopoverBackgroundView *appearance = [SKPopoverBackgroundView appearance];
...
+ (void)load
{
    [SKPopoverController setDefaultTheme:[SKPopoverTheme theme]];
}
...
- (id)init
{
    ...
    if (self)
    {
        ...
        animationDuration = SK_POPOVER_DEFAULT_ANIMATION_DURATION;
        
        themeUpdatesEnabled = NO;
        
        [self setTheme:[SKPopoverController defaultTheme]];
...
- (void)setTheme:(SKPopoverTheme *)value
{
                    ...
                    completion:(void (^)(void))completion
{
    [self presentPopoverFromRect:aRect
        ...
        permittedArrowDirections:aArrowDirections
                        animated:aAnimated
                         options:SKPopoverAnimationOptionFade
...
- (void)presentPopoverFromRect:(CGRect)aRect
      ...
      permittedArrowDirections:(SKPopoverArrowDirection)aArrowDirections
                    ...
                    completion:(void (^)(void))completion
{
    NSAssert((aArrowDirections != SKPopoverArrowDirectionUnknown), @"SKPopoverArrowDirection must not be UNKNOWN");
    ...
    if (overlayView == nil)
    {
        overlayView = [[SKPopoverOverlayView alloc] initWithFrame:inView.window.bounds];
        ...
        backgroundView = [[SKPopoverBackgroundView alloc] initWithContentSize:contentViewSize];
    ...
    if (animated)
    {
        if ((options & SKPopoverAnimationOptionFade) == SKPopoverAnimationOptionFade)
        {
            overlayView.alpha = 0;
        ...
        if ((options & SKPopoverAnimationOptionScale) == SKPopoverAnimationOptionScale)
        {
                             ...
                             completion:(void (^)(void))completion
{
    ...
    aArrowDirections = SKPopoverArrowDirectionDown | SKPopoverArrowDirectionUp;
                            ...
                            completion:(void (^)(void))completion
{
    [self presentPopoverFromRect:CGRectZero
                          inView:nil
        permittedArrowDirections:SKPopoverArrowDirectionNone
                        animated:aAnimated
                         options:aOptions
                      completion:completion];
}

- (CGAffineTransform)transformForArrowDirection:(SKPopoverArrowDirection)arrowDirection
{
    ...
    if (backgroundView.arrowHeight > 0)
    {
        ...
        //SK_LOG(@"containerView.arrowOffset = %f", containerView.arrowOffset);
        //SK_LOG(@"containerViewSize = %@", NSStringFromCGSize(containerViewSize));
        //SK_LOG(@"orientation = %@", SKStringFromOrientation(orientation));
...
- (void)positionPopover:(BOOL)aAnimated
{
    ...
    CGSize contentViewSize = self.popoverContentSize;
    CGSize minContainerSize = SK_POPOVER_MIN_SIZE;
    ...
    SKPopoverArrowDirection arrowDirection = permittedArrowDirections;
    ...
    viewFrame = SKRectInWindowBounds(viewFrame, orientation);
    
    minX = popoverLayoutMargins.left;
    maxX = overlayWidth - popoverLayoutMargins.right;
    minY = SKStatusBarHeight() + popoverLayoutMargins.top;
    ...
    if (arrowDirection == SKPopoverArrowDirectionDown)
    {
        backgroundView.arrowDirection = SKPopoverArrowDirectionDown;
    ...
    if (arrowDirection == SKPopoverArrowDirectionUp)
    {
        backgroundView.arrowDirection = SKPopoverArrowDirectionUp;
        containerViewSize = [backgroundView sizeThatFits:contentViewSize];
    ...
    if (arrowDirection == SKPopoverArrowDirectionRight)
    {
        backgroundView.arrowDirection = SKPopoverArrowDirectionRight;
        containerViewSize = [backgroundView sizeThatFits:contentViewSize];
    ...
    if (arrowDirection == SKPopoverArrowDirectionLeft)
    {
        backgroundView.arrowDirection = SKPopoverArrowDirectionLeft;
        containerViewSize = [backgroundView sizeThatFits:contentViewSize];
    ...
    if (arrowDirection == SKPopoverArrowDirectionNone)
    {
        backgroundView.arrowDirection = SKPopoverArrowDirectionNone;
        containerViewSize = [backgroundView sizeThatFits:contentViewSize];
    ...
    backgroundView.transform = CGAffineTransformMakeRotation(SKInterfaceOrientationAngleOfOrientation(orientation));
    
    containerFrame = backgroundView.frame;
    
    containerFrame.origin = SKPointRelativeToOrientation(containerOrigin, containerFrame.size, orientation);
...
//    SK_LOG(@"popoverContainerView.frame = %@", NSStringFromCGRect(backgroundView.frame));
}
                  ...
                  callDelegate:(BOOL)callDelegate
{
    ...
    SKPopoverAnimationOptions style = aOptions;
    ...
    if (aAnimated)
    {
        [UIView animateWithDuration:duration animations:^{
            ...
            if (strongSelf)
            {
                if ((style & SKPopoverAnimationOptionFade) == SKPopoverAnimationOptionFade)
                {
                    strongSelf->backgroundView.alpha = 0;
                }
                
                if ((style & SKPopoverAnimationOptionScale) == SKPopoverAnimationOptionScale)
                {
...
- (void)popoverOverlayViewDidTouch:(SKPopoverOverlayView *)aOverlayView
{
...
- (void)popoverBackgroundViewDidTouchOutside:(SKPopoverBackgroundView *)aBackgroundView
{
...
{
    SKPopoverArrowDirection arrowDirection = SKPopoverArrowDirectionUnknown;
    
    NSMutableArray *areas = [NSMutableArray arrayWithCapacity:0];
    SKPopoverArea *area;
    
    if ((arrowDirections & SKPopoverArrowDirectionDown) == SKPopoverArrowDirectionDown)
    {
        area = [[SKPopoverArea alloc] init];
        area.areaSize = [self sizeForRect:aRect inView:aView arrowHeight:arrowHeight arrowDirection:SKPopoverArrowDirectionDown];
        area.arrowDirection = SKPopoverArrowDirectionDown;
        [areas addObject:area];
    }
    
    if ((arrowDirections & SKPopoverArrowDirectionUp) == SKPopoverArrowDirectionUp)
    {
        area = [[SKPopoverArea alloc] init];
        area.areaSize = [self sizeForRect:aRect inView:aView arrowHeight:arrowHeight arrowDirection:SKPopoverArrowDirectionUp];
        area.arrowDirection = SKPopoverArrowDirectionUp;
        [areas addObject:area];
    }
    
    if ((arrowDirections & SKPopoverArrowDirectionLeft) == SKPopoverArrowDirectionLeft)
    {
        area = [[SKPopoverArea alloc] init];
        area.areaSize = [self sizeForRect:aRect inView:aView arrowHeight:arrowHeight arrowDirection:SKPopoverArrowDirectionLeft];
        area.arrowDirection = SKPopoverArrowDirectionLeft;
        [areas addObject:area];
    }
    
    if ((arrowDirections & SKPopoverArrowDirectionRight) == SKPopoverArrowDirectionRight)
    {
        area = [[SKPopoverArea alloc] init];
        area.areaSize = [self sizeForRect:aRect inView:aView arrowHeight:arrowHeight arrowDirection:SKPopoverArrowDirectionRight];
        area.arrowDirection = SKPopoverArrowDirectionRight;
        [areas addObject:area];
    }
    
    if ((arrowDirections & SKPopoverArrowDirectionNone) == SKPopoverArrowDirectionNone)
    {
        area = [[SKPopoverArea alloc] init];
        area.areaSize = [self sizeForRect:aRect inView:aView arrowHeight:arrowHeight arrowDirection:SKPopoverArrowDirectionNone];
        area.arrowDirection = SKPopoverArrowDirectionNone;
        [areas addObject:area];
    }
    ...
    if ([areas count] > 1)
    {
        NSIndexSet* indexes = [areas indexesOfObjectsPassingTest:^BOOL(id obj, NSUInteger idx, BOOL *stop) {
            SKPopoverArea* popoverArea = (SKPopoverArea*)obj;
    ...
    [areas sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
        SKPopoverArea *area1 = (SKPopoverArea *)obj1;
        SKPopoverArea *area2 = (SKPopoverArea *)obj2;
    ...
    {
        SKPopoverArea *popoverArea = (SKPopoverArea *)[areas objectAtIndex:i];
    ...
    if (arrowDirection == SKPopoverArrowDirectionUnknown)
    {
        if ([areas count] > 0)
        {
            arrowDirection = ((SKPopoverArea *)[areas objectAtIndex:0]).arrowDirection;
        }
        else
        {
            if ((arrowDirections & SKPopoverArrowDirectionDown) == SKPopoverArrowDirectionDown)
            {
                arrowDirection = SKPopoverArrowDirectionDown;
            }
            else if ((arrowDirections & SKPopoverArrowDirectionUp) == SKPopoverArrowDirectionUp)
            {
                arrowDirection = SKPopoverArrowDirectionUp;
            }
            else if ((arrowDirections & SKPopoverArrowDirectionLeft) == SKPopoverArrowDirectionLeft)
            {
                arrowDirection = SKPopoverArrowDirectionLeft;
            }
            else
            {
                arrowDirection = SKPopoverArrowDirectionRight;
            }
        }
    }
...
- (CGSize)sizeForRect:(CGRect)aRect
               inView:(UIView *)aView
          arrowHeight:(float)arrowHeight
       arrowDirection:(SKPopoverArrowDirection)arrowDirection
{
    UIInterfaceOrientation orientation = [[UIApplication sharedApplication] statusBarOrientation];
    ...
    else if (arrowDirection == SKPopoverArrowDirectionUp)
    {
        result.width = maxX - minX;
...
static NSString* SKStringFromOrientation(NSInteger orientation) {
...
static float SKStatusBarHeight() {
...
static float SKInterfaceOrientationAngleOfOrientation(UIInterfaceOrientation orientation)
{
    float angle;
...
static CGRect SKRectInWindowBounds(CGRect rect, UIInterfaceOrientation orientation) {
...
static CGPoint SKPointRelativeToOrientation(CGPoint origin, CGSize size, UIInterfaceOrientation orientation) {
...
- (void)keyboardWillShow:(NSNotification *)notification
{
    ...
    //UIInterfaceOrientation orientation = [[UIApplication sharedApplication] statusBarOrientation];
    //SK_LOG(@"orientation = %@", SKStringFromOrientation(orientation));
    //SK_LOG(@"keyboardRect = %@", NSStringFromCGRect(keyboardRect));
