...
- (void)viewDidLoad {
    ...
    [self performSelector:@selector(getLOCWithdrawalDetailsFromServer) withObject:nil afterDelay:0.1];
    ...
    [self.rupeeSymbolLabel setText:CURRENCY_SYMBOL];
}
...
- (void)getLOCWithdrawalDetailsFromServer {
    ...
    [ServerCall getServerResponseWithParameters:dictParam withHUD:YES withHudBgView:self.view withCompletion:^(id response) {
        ...
        {            
            [AlertViewManager sharedManager].alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"", nil) message:response cancelButtonItem:[RIButtonItem itemWithLabel:NSLocalizedString(@"OK", nil) action:^{
            ...
            self.minAmountLabel.text = [NSString stringWithFormat:@"%@%.1f",CURRENCY_SYMBOL,minAmount];
            self.maxAmountLabel.text = [NSString stringWithFormat:@"%@%.1f",CURRENCY_SYMBOL,maxAmount];
...
- (void)calculateExpectedEMI {
    ...
    self.balanceValueLabel.text = [NSString stringWithFormat:@"%@%.1f",CURRENCY_SYMBOL,balance];
